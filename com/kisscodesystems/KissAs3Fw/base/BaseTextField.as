...
*/
    public function getTextIsAtLeastLength ( ) : Boolean
    {
      if ( application != null )
      {
        return application . trim ( text ) . length >= minChars ;
      }
      else
      {
...
// This will be the new text.
      if ( isHtml )
      {
        htmlText = textText == null ? "" : textText ;
      }
      else
      {
        text = textText == null ? "" : textText ;
      }
...
// If this is different from the exisging..
      if ( textCode != newTextCode || newTextCode == "" )
      {
