...
package com . kisscodesystems . KissAs3Fw . ui
{
  ...
  public class Potmet extends BaseSprite
  {
...
// The drawed shapes.
    private var drawShapeLittle : BaseShape = null ;
    private var drawShapeBig : BaseShape = null ;
...
*/
    public function Potmet ( applicationRef : Application ) : void
    {
// Super.
...
// The drawed shapes.
      drawShapeBig = new BaseShape ( application ) ;
      drawContainer . addChild ( drawShapeBig ) ;
      drawShapeLittle = new BaseShape ( application ) ;
      drawContainer . addChild ( drawShapeLittle ) ;
...
*/
    ...
    private function spriteMoverMouseDown ( e : MouseEvent ) : void
    {
      if ( getEnabled ( ) )
      {
        addEventListener ( Event . ENTER_FRAME , updatePotmet ) ;
...
/*
** Sets the precision of the potmet.
*/
...
/*
** Updating the whole potmet.
*/
    private function updatePotmet ( e : Event ) : void
    {
...
*/
    private function moverReset ( ) : void
    {
      ...
      removeEventListener ( Event . ENTER_FRAME , updatePotmet ) ;
      if ( getEnabled ( ) )
      {
...
*/
    private function resize ( e : Event ) : void
    {
      if ( application != null )
      {
        radius = ( application . getPropsDyn ( ) . getTextFieldHeight ( textLabel . getTextType ( ) ) + 2 * application . getPropsDyn ( ) . getAppPadding ( ) ) / 2 ;
        radiusLittle = int ( radius / 3 ) ;
        super . setswh ( application . getPropsDyn ( ) . getTextFieldHeight ( textLabel . getTextType ( ) ) + 2 * application . getPropsDyn ( ) . getAppPadding ( ) , application . getPropsDyn ( ) . getTextFieldHeight ( textLabel . getTextType ( ) ) + 2 * application . getPropsDyn ( ) . getAppPadding ( ) ) ;
        drawShapeLittle . x = radius + application . getPropsDyn ( ) . getAppLineThickness ( ) ;
        drawShapeLittle . y = radius + application . getPropsDyn ( ) . getAppLineThickness ( ) ;
...
*/
    private function updateDisplaying ( val : Number ) : void
    {
      ...
      drawShapeLittle . rotation = 240 * factor ;
...
*/
    ...
    {
      drawShapeLittle . graphics . clear ( ) ;
      drawShapeLittle . graphics . lineStyle ( 0 , 0 , 0 ) ;
      drawShapeLittle . graphics . beginFill ( application . getPropsDyn ( ) . getAppFontColorMid ( ) , 1 ) ;
      drawShapeLittle . graphics . drawCircle ( - radius * Math . cos ( Math . PI / 6 ) + radiusLittle , radius * Math . sin ( Math . PI / 6 ) - radiusLittle , radiusLittle ) ;
      drawShapeLittle . graphics . endFill ( ) ;
      drawShapeBig . graphics . clear ( ) ;
      drawShapeBig . graphics . lineStyle ( application . getPropsDyn ( ) . getAppLineThickness ( ) , application . getPropsDyn ( ) . getAppFontColorMid ( ) , 1 ) ;
      drawShapeBig . graphics . drawCircle ( radius + application . getPropsDyn ( ) . getAppLineThickness ( ) , radius + application . getPropsDyn ( ) . getAppLineThickness ( ) , radius ) ;
