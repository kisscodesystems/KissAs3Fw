...
# Sorts the paths coming in by the name of the resource, that is by the path with the
# extension cut off it. Without that, renaming png to svg reorders every generated member,
# because sort compares "food_beer.png" against "food_beers.png" on the "p" of the extension
# and "food_beer.svg" against "food_beers.svg" on the "v" of it.
function sort_by_name()
{
  awk '{ k = $0; sub(/\.[^.\/]*$/, "", k); print k "\t" $0 }' | sort | cut -f 2-
}
...
function generate_class()
{
  ...
  LISTCOMMAND='find "$BASE" -name "*.$FILEEXT" | sort_by_name'
  LIST2COMMAND='find "$BASE" -type d | sort'
