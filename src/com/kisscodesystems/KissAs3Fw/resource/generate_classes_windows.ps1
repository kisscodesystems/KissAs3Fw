...
function Invoke-GenerateClass([string] $folderName)
{
  ...
  if ($folderName -eq "icon")
  {
    ...
    Add-Target $target "    {"
    Add-Target $target "      // a BitmapData of a zero or of a negative size throws an ArgumentError, and an icon"
    Add-Target $target "      // is drawn while the objects of an application are being built, where that error"
    Add-Target $target "      // would be caught by the constructor of it and take every object of it down"
    Add-Target $target "      var size:int = Math.max(1, iconSize);"
    Add-Target $target "      var bitmapData:BitmapData = new BitmapData(size, size, true, 0x00ffffff);"
    Add-Target $target "      if (bitmap != null)"
    Add-Target $target "      {"
    Add-Target $target "        var matrix:Matrix = new Matrix();"
    Add-Target $target "        matrix.scale(size / bitmap.width, size / bitmap.height);"
  ...
  elseif ($folderName -eq "emoji")
  {
    ...
    Add-Target $target "      {"
    Add-Target $target "        return new BitmapData(Math.max(1, emojiSize), Math.max(1, emojiSize), true, 0x00ffffff);"
    Add-Target $target "      }"
